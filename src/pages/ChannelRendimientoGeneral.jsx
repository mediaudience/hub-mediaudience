...
export default function ChannelRendimientoGeneral({ data, uiTabs }) {
  ...
  // Un anunciante/campaña/motivo elegido puede dejar de existir para el
  // cliente activo si se cambia de cliente arriba -- se limpia para no dejar
  // un filtro "fantasma" que deja la tabla en 0 filas sin ninguna pista de por
  ...
  const motivosDisponibles = useMemo(
    () => [...new Set(filasDelClienteActivo.map((r) => r.motivo).filter(Boolean))].sort(),
    [filasDelClienteActivo]
  );
  ...
  const filas = useMemo(
    ...
    [filasDelClienteActivo, campana, anunciante, motivo, periodo]
  );
  ...
  const filters = [
    ...
    ...(tieneCampanaAnunciante
      ? [
          ...
          { label: "Motivo", options: motivosDisponibles, value: motivo, onChange: setMotivo },
        ]
      : []),
  ];
